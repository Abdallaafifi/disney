...
const Series = () => {
  ...
  const item = data.find((i) => i.original_name === id);
  ...
  const GetVideo = async () => {
    try {
      axios
        .get(
          `https://api.themoviedb.org/3/tv/${item?.id}/videos?api_key=632263edb588b456786ff279e4e12517&language=en-US`
        )
            ...
            <div className="flex items-center flex-wrap md-my-0 my-2">
              ...
              <button
                ...
                className={`flex items-center md:text-[20px] text-[12px]   transition rounded-full md:p-4 p-[12px]  ml-2   font-bold ${
                  add ? "bg-gray-100 text-black " : "bg-transparent text-white"
                }`}
              >
